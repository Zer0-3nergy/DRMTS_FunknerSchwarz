// -------------------------------------------------------------
// 
// File Name: F:\MyGit_Folder\DRMTS_FunknerSchwarz\behav\Verilog\dsm_l2_sim_deci_cic_HDLgeneration\Addressable_Delay_Line_block10.v
// Created: 2026-01-26 17:23:30
// 
// Generated by MATLAB 25.2, HDL Coder 25.2, and Simulink 25.2
// 
// -------------------------------------------------------------


// -------------------------------------------------------------
// 
// Module: Addressable_Delay_Line_block10
// Source Path: dsm_l2_sim_deci_cic_HDLgeneration/DECIMATOR/HBF2/FIRPartlySerial/Addressable Delay Line
// Hierarchy Level: 3
// Model version: 17.86
// 
// Addressable Delay Line
// 
// -------------------------------------------------------------

`timescale 1 ns / 1 ns

module Addressable_Delay_Line_block10
          (clk,
           reset,
           enb,
           dataIn,
           validIn,
           rdAddr,
           dataOut);


  input   clk;
  input   reset;
  input   enb;
  input   signed [66:0] dataIn;  // sfix67_En62
  input   validIn;
  input   rdAddr;  // ufix1
  output  signed [66:0] dataOut;  // sfix67_En62


  wire rdCompare;
  reg signed [66:0] delayedSignals0;  // sfix67_En62
  wire signed [66:0] ZEROCONST;  // sfix67_En62
  wire signed [66:0] switchDataOut;  // sfix67_En62
  reg signed [66:0] dataOut_1;  // sfix67_En62


  assign rdCompare = rdAddr > 1'b0;

  always @(posedge clk or posedge reset)
    begin : delay0_process
      if (reset == 1'b1) begin
        delayedSignals0 <= 67'sh00000000000000000;
      end
      else begin
        if (enb && validIn) begin
          delayedSignals0 <= dataIn;
        end
      end
    end

  assign ZEROCONST = 67'sh00000000000000000;

  assign switchDataOut = (rdCompare == 1'b0 ? delayedSignals0 :
              ZEROCONST);

  always @(posedge clk or posedge reset)
    begin : dataOutReg_process
      if (reset == 1'b1) begin
        dataOut_1 <= 67'sh00000000000000000;
      end
      else begin
        if (enb) begin
          dataOut_1 <= switchDataOut;
        end
      end
    end

  assign dataOut = dataOut_1;

endmodule  // Addressable_Delay_Line_block10

